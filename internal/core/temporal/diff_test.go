...
func TestCompareHandlesChangesAddsRemovesAndCounterReset(t *testing.T) {
	...
	if len(diff.Changes) != 4 { t.Fatalf("changes=%#v", diff.Changes) }
...
func TestCompareRejectsDifferentTargets(t *testing.T) {
	...
	if _, err := Compare(a, b); err == nil { t.Fatal("expected target mismatch") }
}
...
func assertChange(t *testing.T, diff Diff, key string, kind ChangeKind, delta *float64) {
	...
	for _, change := range diff.Changes {
		...
		if change.Kind != kind { t.Fatalf("%s kind=%s", key, change.Kind) }
		if delta == nil && change.NumericDelta != nil { t.Fatalf("%s unexpected delta", key) }
		if delta != nil && (change.NumericDelta == nil || *change.NumericDelta != *delta) { t.Fatalf("%s delta=%v", key, change.NumericDelta) }
		return
	}
