...
func (r redundantIndexRule) Evaluate(ctx finding.AnalysisContext) []finding.Finding {
	...
	for _, shortID := range ids {
		...
		for _, longID := range ids {
			...
			shortKey, longKey, separator := a.columns, b.columns, ","
			if a.hasSignature && b.hasSignature {
				shortKey, longKey, separator = a.signature, b.signature, ";"
			}
			if len(shortKey) >= len(longKey) {
				continue
			}
			if longKey == shortKey || strings.HasPrefix(longKey, shortKey+separator) {
